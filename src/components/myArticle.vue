		...
		<div id="centerContain">
			<div class="choosePart">
				...
				<div class="chooseBottom">
					<div class="chooseBox" v-if="primission">
					...
					<div class="changePickType">
						<span v-bind:class='isPassPart?"pass-color":"pickItem"' v-on:click="changePickType('p')">审核通过</span>
                        <span v-bind:class='isWaitPart?"wait-color":"pickItem"' v-on:click="changePickType('div')">待审核</span>
						<span v-bind:class='isFailPart?"fail-color":"pickItem"' v-on:click="changePickType('span')">审核失败</span>
					</div>
				</div>
			</div>
			<div class="searchResult">
					<div class="searchResultContain">
						<div class="searchResultContainBox" v-show="showSwtich == result.checkStatus " v-for="result in searchResults" :key="result.index">
							<div class="searchResultIntro">
								<span style="width:100%">
                                    <div style = "width:100%" v-bind:class="checkStatusJudge(result.checkStatus)">[{{result.checkStatus}}]</div>
...
export default {
    ...
    methods:{
        ...
        checkStatusJudge:function(stauts){
            if(stauts == "通过"){
                return "pass-color"; 
            }
            if(status == "未通过"){
...
.changePickType .pass-color:hover {
	color: yellowgreen!important;
	opacity: 1;
	cursor: pointer;
}
.changePickType .pass-color:before{
	content: "[";
	opacity: 0;
}
.changePickType .pass-color:after{
	content: "]";
	opacity: 0;
}
.changePickType .pass-color:hover:before,
.changePickType .pass-color:hover:after {
	transition: 0.5s;
	opacity: 1;
}


.changePickType .fail-color:hover {
	color: yellowgreen!important;
	opacity: 1;
	cursor: pointer;
}
.changePickType .fail-color:before{
	content: "[";
	opacity: 0;
}
.changePickType .fail-color:after{
	content: "]";
	opacity: 0;
}
.changePickType .fail-color:hover:before,
.changePickType .fail-color:hover:after {
	transition: 0.5s;
	opacity: 1;
}


.changePickType .wait-color:hover {
	color: yellowgreen!important;
	opacity: 1;
	cursor: pointer;
}
.changePickType .wait-color:before{
	content: "[";
	opacity: 0;
}
.changePickType .wait-color:after{
	content: "]";
	opacity: 0;
}
.changePickType .wait-color:hover:before,
.changePickType .wait-color:hover:after {
	transition: 0.5s;
	opacity: 1;
}
</style>
